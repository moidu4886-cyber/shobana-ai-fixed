...
# ── AI Search (free providers supported) ────────────────────────────────────
# AI_PROVIDER options: gemini (free), groq (free), openrouter (free), anthropic (paid)
# Default is gemini — get a free key at https://aistudio.google.com/app/apikey
AI_PROVIDER        = environ.get('AI_PROVIDER', 'openrouter')
AI_API_KEY         = environ.get('AI_API_KEY', '')
AI_SEARCH_ENABLED  = is_enabled(environ.get('AI_SEARCH_ENABLED', 'false'), False)
# Legacy alias — kept so old deployments with ANTHROPIC_API_KEY still work
if not AI_API_KEY:
    AI_API_KEY = environ.get('ANTHROPIC_API_KEY', '')
# Bot settings
BOT_START_TIME = time()
CACHE_TIME = int(environ.get('CACHE_TIME', 300))
USE_CAPTION_FILTER = bool(environ.get('USE_CAPTION_FILTER', False))
PICS = (environ.get('PICS', 'https://cdn.corenexis.com/files/c/9164811720.jpg')).split()
...
SUPPORT_CHAT = environ.get('SUPPORT_CHAT', 'ottverse_in')
