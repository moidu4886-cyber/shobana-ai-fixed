...
async def give_filter(client, message):
    try:
        ...
        logger.warning("Failed to delete message: %s", e)  # 403 is non-fatal, use warning not exception
...
#ALERT FN IN SPELL CHECK FOR LANGAUGES TO KNOW HOW TO TYPE MOVIES esp english spell check goto adv spell check to check donot change the codes      
        ...
        buttons = [[
            ...
            InlineKeyboardButton(f'ᴏᴛᴛ ᴜᴘᴅᴀᴛᴇs​', url='https://t.me/ottverse_in'),
...
async def auto_filter(client, msg, spoll=False):
    if not spoll:
        ...
        if 2 < len(message.text) < 100:
            ...
            # ── Log search activity for AI analytics ──────────────────────
            _uid = message.from_user.id if message.from_user else 0
            _cid = message.chat.id
            asyncio.create_task(log_search(_uid, _cid, search, result_found=bool(files)))
            # ─────────────────────────────────────────────────────────────

            if not files:
                # ── Fallback order (FIXED):                              ──
                # 1. AI intent search  — runs for ALL messages when enabled
                #    (AI parses intent → DB search → real results only)
                # 2. Spell check       — secondary fallback if AI found nothing
                # ──────────────────────────────────────────────────────────
                from info import AI_SEARCH_ENABLED
                if AI_SEARCH_ENABLED:
                    from plugins.ai_search import ai_smart_search
                    triggered = await ai_smart_search(client, message, search)
                    if triggered:
                        return
                # Spell check runs only if AI is disabled or returned nothing
