...
# Added by: AI Search Fix (mn-bots)
# Architecture: AI parses intent → this function queries DB → DB is source of truth

async def get_search_results_by_intent(intent: dict, max_results: int = 10, offset: int = 0):
    """
    Search the database using a structured intent dict produced by the AI parser.

    intent keys (all optional):
        movie_name : str  — cleaned movie title
        language   : str  — e.g. "hindi", "tamil", "malayalam"
        year       : str  — 4-digit year
        quality    : str  — e.g. "720p", "1080p", "4k"
        tags       : list — actor names, genre keywords, etc.

    Returns (files, next_offset, total_results) — same shape as get_search_results.
    """
    movie_name = (intent.get("movie_name") or "").strip()
    language   = (intent.get("language")   or "").strip().lower()
    year       = (intent.get("year")       or "").strip()
    quality    = (intent.get("quality")    or "").strip()
    tags       = [t.strip().lower() for t in (intent.get("tags") or []) if t and len(str(t).strip()) > 1]

    # ── If nothing at all was parsed, bail out early ──────────────────────────
    if not any([movie_name, language, year, quality, tags]):
        return [], "", 0

    # ══ SQL (PostgreSQL) path ═════════════════════════════════════════════════
    if not USE_MONGO:
        where_parts = []
        params: dict = {"offset": int(offset), "limit": int(max_results)}

        def _ilike_block(value: str, key_prefix: str) -> str:
            """Return SQL condition for file_name (and optionally caption) ILIKE %value%."""
            k = f"p_{key_prefix}"
            params[k] = f"%{value}%"
            if USE_CAPTION_FILTER:
                return f"(file_name ILIKE :{k} OR COALESCE(caption,'') ILIKE :{k})"
            return f"file_name ILIKE :{k}"

        # movie_name — each word must appear (AND of words)
        if movie_name:
            words = [w for w in movie_name.split() if len(w) > 1]
            for i, word in enumerate(words[:6]):
                where_parts.append(_ilike_block(word, f"mn_{i}"))

        # language
        if language:
            where_parts.append(_ilike_block(language, "lang"))

        # quality
        if quality:
            where_parts.append(_ilike_block(quality, "qual"))

        # year
        if year and year.isdigit() and len(year) == 4:
            where_parts.append(_ilike_block(year, "year"))

        # tags — only applied when no movie_name (prevents over-filtering)
        if tags and not movie_name:
            tag_clauses = []
            for i, tag in enumerate(tags[:5]):
                tk = f"p_tag_{i}"
                params[tk] = f"%{tag}%"
                if USE_CAPTION_FILTER:
                    tag_clauses.append(f"(file_name ILIKE :{tk} OR COALESCE(caption,'') ILIKE :{tk})")
                else:
                    tag_clauses.append(f"file_name ILIKE :{tk}")
            if tag_clauses:
        ...
        with store.begin() as conn:
            total_results = int(
                conn.execute(text(f"SELECT COUNT(*) FROM media WHERE {where_clause}"), params).scalar() or 0
            )
            rows = conn.execute(
                text(
                    f"""
    ...
    def _name_or_caption_regex(pattern_str: str):
        """Return a $or condition for file_name / caption regex."""
        try:
            rx = re.compile(pattern_str, re.IGNORECASE)
        except re.error:
            rx = re.compile(re.escape(pattern_str), re.IGNORECASE)
        if USE_CAPTION_FILTER:
            return {"$or": [{"file_name": rx}, {"caption": rx}]}
        return {"file_name": rx}

    and_conditions = []

    # movie_name — words must all appear (AND)
    if movie_name:
        words = [re.escape(w) for w in movie_name.split() if len(w) > 1]
        if words:
            for word_pattern in words[:6]:
                and_conditions.append(_name_or_caption_regex(word_pattern))

    # language
    if language:
        and_conditions.append(_name_or_caption_regex(re.escape(language)))

    # quality
    if quality:
        and_conditions.append(_name_or_caption_regex(re.escape(quality)))

    # year
    if year and year.isdigit() and len(year) == 4:
        and_conditions.append(_name_or_caption_regex(re.escape(year)))

    # tags — OR across tags, applied only when no movie_name
    if tags and not movie_name:
        try:
            tag_rx = re.compile("|".join(re.escape(t) for t in tags[:5]), re.IGNORECASE)
            if USE_CAPTION_FILTER:
                and_conditions.append({"$or": [{"file_name": tag_rx}, {"caption": tag_rx}]})
            else:
                and_conditions.append({"file_name": tag_rx})
        except re.error:
            pass
    ...
    mongo_filter = {"$and": and_conditions} if len(and_conditions) > 1 else and_conditions[0]

    fetch_limit = max(offset + max_results, max_results)

    count_tasks = [col.count_documents(mongo_filter) for col in _mongo_collections]
    total_results = sum(await asyncio.gather(*count_tasks))
    next_offset = offset + max_results
    if next_offset >= total_results:
        next_offset = ""
    ...
    async def _fetch(col):
        docs = await col.find(mongo_filter).sort("created_at", -1).limit(fetch_limit).to_list(length=fetch_limit)
        return [_as_media_doc(d) for d in docs]
